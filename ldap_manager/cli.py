...
class _ErrorHandlingGroup(click.Group):
    """Click group that catches exceptions from all subcommands.

    Converts LDAP errors, file errors, YAML errors, and other
    exceptions into clean one-line messages instead of tracebacks.
    Uses structured exit codes (see errors.py) so scripts can
    distinguish error categories without parsing text.

    Respects ``--debug`` for development: when set, re-raises the
    original exception so the full traceback is visible.

    Applied only to the top-level ``main`` group — exceptions from
    every nested subcommand propagate up through Click's group
    hierarchy and get caught here.
    """
    ...
    def invoke(self, ctx: click.Context) -> None:
        try:
            ...
            if ctx.params.get("debug") or os.environ.get("LDAP_MANAGER_DEBUG"):
                raise
            from .errors import exit_code_for, format_error
...
def _audit(ctx: click.Context, action: str, target: str, **kwargs: Any) -> None:
    """Emit an audit log entry. Called after every mutating operation."""
    logger = ctx.obj.get("audit")
    if logger:
...
def user_create(
    ...
    _audit(ctx, "user.create", dn, details={"uid": uid})
    if generated_pw:
...
def user_update(ctx: click.Context, uid: str, attrs: tuple[str, ...]) -> None:
    ...
    _audit(ctx, "user.update", uid, details={"attrs": list(parsed.keys())})
...
def user_enable(ctx: click.Context, uid: str, as_json: bool) -> None:
    ...
    _audit(ctx, "user.enable", uid)


@user.command("passwd")
@click.argument("uid")
@click.option("--random", "use_random", is_flag=True, help="Generate a random password instead of prompting.")
@click.option("--stdin", "use_stdin", is_flag=True, help="Read password from stdin (for piping).")
@click.pass_context
def user_passwd(ctx: click.Context, uid: str, use_random: bool, use_stdin: bool) -> None:
    ...
    EXAMPLES
        ...
        # Generate random password, print to stderr
        ldap-manager user passwd jdoe --random

        # Read from pipe
        echo "S3cret!" | ldap-manager user passwd jdoe --stdin

    \b
    NOTES
        Password is hashed with the configured scheme before storing.
        For bulk password changes, use 'passwd-all' instead.
    """
    if use_random and use_stdin:
        click.echo("error: --random and --stdin are mutually exclusive.", err=True)
        sys.exit(2)
    ...
    if use_random:
        ...
        if not pw:
            click.echo("error: no password received on stdin.", err=True)
            sys.exit(1)
    else:
    ...
    _audit(ctx, "user.passwd", uid)
    if use_random:
        click.echo(f"Generated password: {pw}", err=True)
...
def backup_dump(ctx: click.Context, no_compress: bool, tag: str) -> None:
    ...
    _audit(ctx, "backup.dump", str(path), details={"tag": tag})
...
def backup_restore(ctx: click.Context, backup_path: str, with_config: bool, yes: bool) -> None:
    ...
    _audit(ctx, "backup.restore", backup_path, details={"with_config": with_config})
...
def passwd_all(
    ...
    _audit(ctx, "passwd_all", "all", details={"rotated": result.rotated, "dry_run": dry_run})
...
def group_add_member(ctx: click.Context, group_cn: str, uid: str) -> None:
    ...
    _audit(ctx, "group.add_member", group_cn, details={"uid": uid})
...
def group_remove_member(ctx: click.Context, group_cn: str, uid: str) -> None:
    ...
    _audit(ctx, "group.remove_member", group_cn, details={"uid": uid})
...
def tree_delete_ou(ctx: click.Context, dn: str, recursive: bool, yes: bool) -> None:
    ...
    _audit(ctx, "tree.delete_ou", dn, details={"recursive": recursive, "count": count})
...
@main.command("doctor")
@click.option("--fix", is_flag=True, help="Auto-fix safe issues (directories, OUs, config defaults).")
@click.pass_context
def doctor_cmd(ctx: click.Context, fix: bool) -> None:
    """Diagnose server, config, and schema issues.

    \b
    Checks connection, base DN, standard OUs, schemas/overlays,
    locked accounts, slapd status, filesystem paths, and config.

    \b
    EXAMPLES
        ldap-manager doctor             # diagnose only
        ldap-manager doctor --fix       # fix safe issues automatically

    \b
    SAFE FIXES (--fix):
        Create missing directories (backup, audit)
        Create missing standard OUs (People, Groups)
        Fix config file permissions (chmod 600)

    \b
    NEVER AUTO-FIXED:
        Schema/overlay loading, locked accounts, server process
    """
    from .doctor import run_doctor

    cfg = ctx.obj["config"]
    # Try to find the config file path for permission checks
    config_path = ctx.parent.params.get("config_path") if ctx.parent else None
    sys.exit(run_doctor(cfg, config_path=config_path, do_fix=fix))
