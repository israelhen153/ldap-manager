...
class UserManager:
    ...
    def create_user(
        ...
        *,
        ...
        password_to_hash = explicit_password
        if explicit_password is None:
            if self._ucfg.generate_password_on_create:
                ...
                log.warning(
                    "Creating user %s with no password — "
                    "generate_password_on_create is off and default_password is empty. "
                    "User will not be able to authenticate until a password is set.",
                    uid,
                )
        ...
        if password_to_hash:
            entry["userPassword"] = [_hash_password(password_to_hash, self._resolve_scheme(backend))]

        add_list = modlist.addModlist(entry)
        backend.add(dn, add_list)
        log.info("Created user %s (%s)", uid, dn)
        return dn, generated_password or explicit_password
