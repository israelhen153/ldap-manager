...
class GroupManager:
    ...
    def get_group(self, backend: Backend, cn: str) -> GroupEntry | None:
        ...
        search_filter = f"(&(|(objectClass=posixGroup)(objectClass=groupOfNames))(cn={safe_cn}))"
        try:
    ...
    def create_group(
        ...
        *,
        ...
        dn = f"cn={_escape(cn)},{self._lcfg.groups_ou}"
    ...
    def get_user_groups(self, backend: Backend, uid: str) -> list[GroupEntry]:
        ...
        member_dn = f"uid={uid},{self._lcfg.users_ou}"
        safe_dn = ldap.filter.escape_filter_chars(member_dn)
        search_filter = f"(|(&(objectClass=posixGroup)(memberUid={safe_uid}))(&(objectClass=groupOfNames)(member={safe_dn})))"
        try:
            results = backend.search(self._lcfg.groups_ou, ldap.SCOPE_SUBTREE, search_filter, _GROUP_ATTRS)
        except ldap.NO_SUCH_OBJECT:
            return []
...
def _escape(value: str) -> str:
    """Escape DN-special characters (RFC 4514)."""
    for char in ("\\", ",", "+", '"', "<", ">", ";", "=", "\0"):
