...
from .validation import validate_uid
    ...
    if action in ("create", "update"):
        ...
        for entry in entries:
            ...
            if stop_on_error and result.failed:
                break
        # Warn if the file had data rows but nothing was processed — likely
        # a delimiter mismatch (e.g. tabs in a .csv file).
        if entries and result.total == 0:
            log.warning(
                "File had %d entries but none were processed — check format and column headers",
                len(entries),
            )
    else:
        ...
        for uid in uids:
            ...
            if stop_on_error and result.failed:
                break
        if uids and result.total == 0:
            log.warning(
                "File had %d UIDs but none were processed",
                len(uids),
            )
...
def _run_one(
    ...
    # Validate uid format on create — don't let bad UIDs into LDAP.
    # Skip validation on delete/enable/disable — those need to operate
    # on entries that may predate validation rules.
    if action == "create":
        try:
            validate_uid(uid)
